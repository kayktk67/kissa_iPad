...
UICollectionViewDelegate {
    ...
    func collectionView(_ collectionView: UICollectionView,
        ...
        let defaultPlace6 = DBRef.child("table/order").child(number[indexPath.row]).child("s1amount")
        defaultPlace6.observe(.value) { (snap: DataSnapshot) in s1amountlabel.text = (snap.value! as AnyObject).description}
        let defaultPlace7 = DBRef.child("table/order").child(number[indexPath.row]).child("s2amount")
        defaultPlace7.observe(.value) { (snap: DataSnapshot) in s2amountlabel.text = (snap.value! as AnyObject).description}
        let defaultPlace8 = DBRef.child("table/order").child(number[indexPath.row]).child("s3amount")
        defaultPlace8.observe(.value) { (snap: DataSnapshot) in s3amountlabel.text = (snap.value! as AnyObject).description}
