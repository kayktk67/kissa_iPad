...
UICollectionViewDelegate {
    ...
    func collectionView(_ collectionView: UICollectionView,
        ...
        let de4amountlabel = Cell.contentView.viewWithTag(28) as! UILabel
        ...
        let TBStatuslabel = Cell.contentView.viewWithTag(27) as! UILabel
        ...
        if Int(self.tbstatus[indexPath.row]) == 1{
            TBStatuslabel.backgroundColor = UIColor.magenta
        }else if Int(self.tbstatus[indexPath.row]) == 2{
            TBStatuslabel.backgroundColor = UIColor.cyan
        }else{
            TBStatuslabel.backgroundColor = UIColor.white
        }
    ...
    @IBAction func TBStatus(sender: UIButton) {
        if let indexPath = collectionView.indexPath(for: sender.superview!.superview as! UICollectionViewCell) {
            if Int(self.tbstatus[indexPath.row]) == 1{
                self.DBRef.child("table/tbstatus").child(self.number[indexPath.row]).setValue(2)
            }else if Int(self.tbstatus[indexPath.row]) == 2{
                self.DBRef.child("table/tbstatus").child(self.number[indexPath.row]).setValue(1)
            }
        }
    }
    ...
    func statusloaddata(){
        for i in 0..<18{
            //席ステータス取得
            let defaultPlace00 = self.DBRef.child("table/status").child(self.number[i])
            ...
            let defaultPlace01 = self.DBRef.child("table/bstatus").child(self.number[i])
            defaultPlace01.observeSingleEvent(of: .value, with: { (snapshot) in self.bstatus[i] = (snapshot.value! as AnyObject).description
            })
            let defaultPlace09 = self.DBRef.child("table/tbstatus").child(self.number[i])
            defaultPlace09.observeSingleEvent(of: .value, with: { (snapshot) in self.tbstatus[i] = (snapshot.value! as AnyObject).description
            })
            let defaultPlace02 = self.DBRef.child("table/sstatus").child(self.number[i])
            defaultPlace02.observeSingleEvent(of: .value, with: { (snapshot) in self.sstatus[i] = (snapshot.value! as AnyObject).description
            })
            let defaultPlace03 = self.DBRef.child("table/dstatus").child(self.number[i])
            defaultPlace03.observeSingleEvent(of: .value, with: { (snapshot) in self.dstatus[i] = (snapshot.value! as AnyObject).description
            })
            let defaultPlace05 = self.DBRef.child("table/dxstatus").child(self.number[i])
            defaultPlace05.observeSingleEvent(of: .value, with: { (snapshot) in self.dxstatus[i] = (snapshot.value! as AnyObject).description
            })
            let defaultPlace04 = self.DBRef.child("table/destatus").child(self.number[i])
            defaultPlace04.observeSingleEvent(of: .value, with: { (snapshot) in self.destatus[i] = (snapshot.value! as AnyObject).description
            })
            let defaultPlaceT2 = self.DBRef.child("table/order").child(self.number[i]).child("completetime")
    ...
    func amountloaddata(){
        for i in 0..<18{
            ...
            let defaultPlace3 = self.DBRef.child("table/setamount").child(self.number[i]).child("sset")
